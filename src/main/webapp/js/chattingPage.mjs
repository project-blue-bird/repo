...
// const ws = new WebSocket("ws://192.168.240.46:9099/chat");
//
// window.onload = () => {
...
function printDate() {
    let now = new Date();
    let day = now.getDate();
    let month = now.getMonth() + 1; // 월은 0부터 시작하므로 1을 더함
    let mm = now.getMinutes().toString().padStart(2, '0'); // 분을 두 자리로 포맷
    let hh = now.getHours();
    let period = hh >= 12 ? 'PM' : 'AM'; // AM/PM 결정
    hh = hh % 12; // 12시간제 변환
    hh = hh ? hh : 12; // 시가 0일 경우 12로 변환
    return `${month}월 ${day}일 (${period}) ${hh}시 ${mm}분`;
}
